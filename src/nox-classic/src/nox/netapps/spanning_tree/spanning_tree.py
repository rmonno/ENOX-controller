...
class Spanning_Tree(Component):
    ...
    def __info(self, text):
        stp_logger.info('%s' % (str(text),))

    def __error(self, text):
        stp_logger.error('%s' % (str(text),))
    ...
    def install(self):
        ...
        self.__info("Spanning tree installed!")

    def dp_join(self, dp, stats):
        self.__info("Datapath join: %x" % (dp,))
        if (not self.datapaths.has_key(dp)):
            ...
            for port in stats['ports']:
                ...
                if port[core.PORT_NO] <= openflow.OFPP_MAX:
                    ...
                    self.ctxt.send_port_mod(dp,
                                            port[core.PORT_NO],
                                            ethernetaddr(hw_addr),
                                            openflow.OFPPC_NO_FLOOD,
                                            openflow.OFPPC_NO_FLOOD)
            # Record the datapath
    ...
    def dp_leave(self, dp):
        self.__info("Datapath leave: %x" % (dp,))
    ...
    def update_spanning_tree_callback(self, links):
        ...
        '''
        # Walk through the datapaths and mark all ports potentially enabled
        now = time.time()
        ...
        while len(srcs) > 0:
            ...
            if not my_links.has_key(src_dpid):
                self.__error("Cannot find src_dpid %x in my_links" % (src_dpid,))
                continue
        ...
        for dp in self.datapaths.iterkeys():
            ...
            for port_no, port in self.datapaths[dp].iteritems():
                if port_no <= openflow.OFPP_MAX:
                    if port['enable'] != port['flood']:
                        ...
                        self.__info("%s port: %x:::%d" % (msg, dp, port_no,))
                        hw_addr = "\0\0" + port[core.HW_ADDR]
                        hw_addr = struct.unpack("!q", hw_addr)[0]
            ...
            self.__info("Ports for %x: Flood: %s Non-flood: %s" % (dp, floodports, nonfloodports,))

    def build_link_dict(self, links):
        '''Build a dictionary of links based on source dpid
        Dict: {src_dpid: {dst_dpid: [(src_port, dst_port), ...]}}'''
        my_links = {}
        ...
        for src_dpid in links.keys():
            ...
            for dst_dpid in links[src_dpid].keys():
                ...
                for (src_port, dst_port) in links[src_dpid][dst_dpid]:
                    ...
                    if not ok:
                        self.__error("Unidir link detected between %x:::%d <--> %x:::%d" %
